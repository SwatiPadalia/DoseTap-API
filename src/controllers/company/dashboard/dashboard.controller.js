...
export const all = async (req, res) => {
  try {
    ...
    const caretaker = await UserCareTakerMappings.findAndCountAll({
      ...
      attributes: [
        [
          sequelize.fn("DISTINCT", sequelize.col("caretaker_id")),
          "caretaker_id",
        ],
      ],
    });
    ...
    const doctor = await DeviceUserMapping.findAll({
      where: {
        company_id,
      },
    ...
    let uniqueDoctors = [...new Set(doctor.map((item) => item.doctor_id))];
    
    data.doctor = uniqueDoctors.length;
    ...
    data.avg_adherence_open =
      adherence_open_count / total ? (adherence_open_count / total) * 100 : 0;
    data.avg_adherence_missed =
      adherence_missed_count / total
        ? (adherence_missed_count / total) * 100
        : 0;
